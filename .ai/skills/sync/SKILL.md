...
description: "Run /sync as the last step after a change is complete, around merge, to keep durable knowledge current. Updates root and nested MEMORY.md, reconciles the scope from repo evidence, and flags specs the change made stale. Surgical edits only: it adds lines, and rewrites single lines it owns. Never a whole section, never curated prose."
---
...
Closes the loop on a completed change: syncs MEMORY.md files, the scope, and linked spec `**Status**:` lines to what the repo now shows, and flags what it must not edit (stale specs, curated prose). The Boundaries table below is the exact contract.
...
**Canonical file:** durable context lives in the tool agnostic **`MEMORY.md`**. **`MEMORY.md`** and **`MEMORY.md`** are regenerated by Boost; never write content into them. /sync edits MEMORY.md, treating MEMORY.md/MEMORY.md as read only.

## Boundaries (these keep the skill from sprawling)

| Action | /sync | Owner |
|---|---|---|
| Edit existing root/nested MEMORY.md | ✅ maintains | /sync |
| Reconcile root MEMORY.md's `## Build approach` line to the scope header's approach (surgical single line edit, like the stack) | ✅ maintains; flags a curated divergence | /sync |
| Add a one line pointer to `design.md` (the UI design system) in the nearest MEMORY.md when a change establishes one | ✅ maintains (pointer only) | /sync |
| Create nested `<area>/MEMORY.md` for an area **net new in this change** | ✅ creates (diff = full area context) + adds root pointer | /sync |
| Create nested doc for an **already existing** undocumented area (only sliced by the diff) | ❌ flags "run /audit" | /audit |
| Create or restructure the **root** MEMORY.md | ❌ flags "run /audit" | /audit |
...
| Reconcile the scope, for the **relevant workspace's** scope file only (not all of `docs/scope/`), tick **any** completed sub task from repo **evidence** (code, tests, MEMORY.md), advance status | ✅ corrects | /sync |
| Add / reorder features or sub tasks in the scope | ❌ leaves alone | /scope |
| Overwrite or rewrite curated MEMORY.md prose | ❌ flags conflict instead | human |
...
- **Drop documentation and config** (`MEMORY.md` at any level, `docs/**`, `*.md`, `test-preferences.json`, lock files, generated output); /sync reads these as targets/context, never as a change source.
...
- Note whether a root `MEMORY.md` exists.
- Find every `MEMORY.md` (root + nested), excluding `node_modules/` and `.git/`.
...
Note the **paths** plus the changed file list and diff command; read the files at write time. Read root MEMORY.md contents now (short and useful to anchor on). For each changed file, note its nearest enclosing directory with a `MEMORY.md` (root or nested); that's the context file most likely to need an update.
...
- Filter out anything already covered by installed skills, connected MCPs, or `MEMORY.md` declined entries.
...
The inputs to apply:
  ...
  3. `ROOT_AGENTS_MD` (root MEMORY.md contents), `NESTED_PATHS` (nested MEMORY.md paths)
...
**MEMORY.md created** (new area):
- `<area>/MEMORY.md`, <what conventions it captures> (+ root pointer added)
