...
**Canonical file:** durable context lives in the tool agnostic **`MEMORY.md`**; **`MEMORY.md`** imports MEMORY.md (the Boost generated guidelines) via the `@` directive. Never write content into MEMORY.md or MEMORY.md, they are regenerated by Boost. Always write durable knowledge into `MEMORY.md`. When you create a new nested `MEMORY.md`, you may create a sibling `MEMORY.md` pointing to MEMORY.md:
...
## Existing context files (you may EDIT these; you may CREATE a nested MEMORY.md + its MEMORY.md pointer for a net new area only)

- **Root MEMORY.md (inlined)**:

ROOT_AGENTS_MD

- **Nested MEMORY.md paths**: NESTED_PATHS
...
### 1. Update existing MEMORY.md files (only where the change made them inaccurate)

Read the diff. For each existing root/nested MEMORY.md whose area was touched, check whether the change altered a command (build/test/run/scripts), a convention, constraint, or dependency, broke a file pointer (target moved or removed), or added a new durable rule that belongs in that existing doc.
...
**Agent Skill and MCP records:** if `INSTALLED_SKILLS_OR_NONE`, `MCP_SERVERS_OR_NONE`, or `DECLINED_TOOLS_OR_NONE` is not `none`, record it surgically in the most specific relevant MEMORY.md:
- Project wide tools (framework, ORM, styling, core DB, hosting, test runner) belong in root MEMORY.md.
- Area specific tools (payments, auth, email, uploads, search, queues) belong in that area's nested MEMORY.md when one exists; otherwise add a short root note and flag the area under `CONTEXT_GAPS`.
...
**Design system pointer:** if this change added or established a `design.md` (the UI design system, art direction plus the build mandate) and the nearest MEMORY.md has no pointer to it, add one surgical line so the mandate is discoverable and loads automatically for UI work: `` - Design system: build all UI to `design.md` (art direction and the maximalist product bar); token values live in CSS. `` Put it in root MEMORY.md for a project-wide `design.md`, or the UI area's nested MEMORY.md if the system is area-scoped. Idempotent: skip if a `design.md` pointer already exists. Never paste design.md content into MEMORY.md; it is a pointer, not a copy.
...
- **Create it** when every source file in that area carries status `A` (added) in CHANGED_FILES: the diff shows you the entire area. If any file in the area is `M` (modified), the area already existed: do NOT create. Write a focused doc: local file pointers, local commands, conventions/constraints visible in the new code, links to any governing spec. End it with the one line note: `_Drafted by /sync from the introducing change, worth a quick human pass._` Then add exactly one pointer line to root MEMORY.md under `## Context files`:
  ...
  **Idempotency + missing section**: skip the pointer if already present; if root has no `## Context files` heading, create it (append near the end of root) and add the pointer under it.
  ...
  Also create the sibling **`<area>/MEMORY.md` pointer** (a one line note plus `@MEMORY.md`) so Claude Code picks up the new area too.
...
- **Never create or restructure the root MEMORY.md.** If the repo has no root MEMORY.md at all, that's /audit's job; record under `CONTEXT_GAPS`.
...
- A nested `<area>/MEMORY.md` describing code that no longer exists is orphaned. Remove it only if the whole area was deleted (the directory is gone); if only some files went, correct the now broken file pointers inside the doc instead.
- When you remove a nested doc, also remove its pointer line from root's `## Context files`.
- Fix any file pointer in any MEMORY.md that targets a deleted/moved path.
...
> Step 1's source file filtering (dropping `*.test.*`, `docs/**`) governs what you sync MEMORY.md from; it does not limit reconciliation. Here you may and should inspect test files, MEMORY.md, and config to judge completion.
...
- **Sync (record conventions)** → the area's `MEMORY.md` exists and reflects the feature.
...
AGENTS_CREATED:
- <area>/MEMORY.md, <conventions captured; root pointer added>
