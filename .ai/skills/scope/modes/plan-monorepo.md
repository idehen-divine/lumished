...
- `/scope web <idea>` plans the `web` app; bare `/scope` on a monorepo asks which workspace(s) (or "repo wide") as a panel. Read that workspace's nested `MEMORY.md` for its stack/conventions; apps differ, don't assume one.
