...
You, the main thread, read and follow this when you write the `MEMORY.md` files. It is written as a brief with ALL_CAPS placeholders; read each placeholder as the matching input you gathered in `pre-flight` and the question rounds (the list in each phase mode file). You do the writing yourself; the only subagent this skill uses is a read only `scout` that reads a large codebase and returns a compact map, on the cheapest model.

---

You are running /audit in **PHASE** mode. Use your Read, Bash, Write, and Edit tools freely.

## Context file convention (MEMORY.md is canonical)

Durable context goes in `MEMORY.md` (root or `<area>/MEMORY.md`), tool agnostic, read by every agent. `MEMORY.md` is only a one line pointer that loads MEMORY.md (the Boost generated file); MEMORY.md is the durable knowledge base every skill reads and writes.

Hard rules:
- Never overwrite an existing `MEMORY.md` (possibly authored by the user or another tool). Create only when missing; otherwise propose additions via the diff format.
- `MEMORY.md` and `MEMORY.md` are regenerated by Boost and must not be updated by skills; always write knowledge into `MEMORY.md`.
- Migration (when told `MIGRATE=yes`): copy the legacy `MEMORY.md` content verbatim into a new `MEMORY.md`, then replace `MEMORY.md` with the pointer above. Never discard curated content.
- A `MEMORY.md` that already exists stays untouched; propose additions only.

## Stamp what you write, so curated content is knowable

Every `MEMORY.md` this skill creates ends with the drafted by line in the templates below. It exists so a later run (this skill or `/sync`) can tell what a tool wrote from what a human wrote, instead of guessing.
...
Root `MEMORY.md` carries two mirrored fields. Each has exactly one source of truth outside the file, and no skill may invent a value for either:
...
<!-- "no", or "yes, apps: web, api, …". If yes: each app/package is its own area, give each
     a nested MEMORY.md with its own stack/commands/conventions; root MEMORY.md keeps
...
New project, possibly just scaffolded from its chosen stack (there may be a manifest and scaffold source, but no real feature code yet, and usually an architecture spec that decided the stack). Create a root MEMORY.md encoding the engineer's chosen standards.
...
**Step 2b: Per workspace nested MEMORY.md (monorepo only)**

If `MONOREPO_OR_NO` is `yes`: for each listed workspace (`apps/*`, `packages/*`), read its manifest. Even with no features built, the scaffold declares the workspace's stack and commands; capture them so `/architect` and `/develop` read them from the workspace's own doc (they won't look in root). Write `<workspace>/MEMORY.md` with the nested template (`## Stack` from its manifest, `## Commands` from its scripts, scoped, e.g. `pnpm --filter <name> dev`, root `## Rules` inherited by reference), plus the sibling `<workspace>/MEMORY.md` pointer and a pointer line under root's `## Context files`. Skip an empty placeholder workspace with no manifest.
...
A codebase exists but no MEMORY.md. Explore enough to write an accurate root MEMORY.md.
...
- All existing `MEMORY.md` (excluding `.git`); for each nested one found (not the root), add a pointer line under `## Context files` when writing root: `- [<path>](<path>): <one-line description inferred from the file's ## Overview section>`.
...
Root MEMORY.md exists. Job: (1) check whether root MEMORY.md has what this area needs, (2) create or update the nested MEMORY.md.

**Step 1: Explore the area**

With your file tools, list all files under `SCOPE_OR_AREA`. Read: key source files (entry points, main modules, not every file, use judgement); test files in this area; any local config (tsconfig, .env.example, etc.); the existing root and area MEMORY.md (provided above; Read them if given as paths).

**Step 2: Gap check root MEMORY.md**
...
**Step 3: Nested MEMORY.md**

Warrant (own patterns, not obvious rules, local commands, or constraints a developer needs to know first) vs not (a simple CRUD module with no surprises, or already well covered by root MEMORY.md).

- Missing + warranted → create with the nested template, then add a pointer to root MEMORY.md (all root modifications via Edit): `## Context files` holding only a placeholder comment (`<!-- ... -->`) → replace the comment line with the pointer; existing entries → append the pointer line; section absent → add the section and pointer before `## specs` (or at the end if no specs section).
...
Root MEMORY.md already exists; the codebase is partially documented. Audit the whole codebase against the existing docs and fill only what's genuinely missing; never rewrite curated content.

**Step 1: Read what's documented**

Read the existing root MEMORY.md (provided above; Read it if given as a path) and every nested MEMORY.md (paths provided above).
...
- (b) Undocumented areas: a major area with distinct conventions/gotchas and no nested MEMORY.md. Create the nested doc (nested template + sibling MEMORY.md pointer) and add its root pointer line via Edit (safe to do directly: creating, not overwriting).
- (c) Stale/incomplete nested docs: an existing nested MEMORY.md missing something now true of its area. Return as `PROPOSED_ADDITIONS`; do NOT edit it yourself.
...
<Installed Agent Skills that carry this project's tool conventions. ONE bullet per skill so a
  ...
  here; an area specific skill goes in that area's nested MEMORY.md.>
...
=== ROOT MEMORY.md TEMPLATE END ===

---

## Nested MEMORY.md template
...
- Write the `MEMORY.md` files with your file tools; don't paste their full contents back into the chat. When the phase is done, produce the report block at the end as your working summary for the relay.
...
- Do not create a nested MEMORY.md unless the area genuinely warrants it.
- Root MEMORY.md must stay under ~60 lines. Cut ruthlessly.
- Never overwrite an existing MEMORY.md, propose additions only via the diff format.
