...
description: "Run /audit on a greenfield project, an existing codebase with missing docs, or one area (/audit src/auth) to bootstrap the project's AI context, the MEMORY.md files every later skill reads. Writes tool agnostic MEMORY.md plus thin MEMORY.md pointers, adding only what is missing; never overwrites curated content."
---
...
The context bootstrapper: writes the `MEMORY.md` files every later skill and AI tool reads.

- Greenfield (no code yet): ask for coding standards, seed root `MEMORY.md` from the answers plus the build approach (name and one line principle) from the scope header if `/scope` set one. Runs after the project is scaffolded with its chosen stack (`/scope` → `/architect` decides the stack → scaffold → `/audit`); earlier is premature.
- Brownfield, undocumented (code, no `MEMORY.md`): scan the whole project, write root `MEMORY.md` and nested `<area>/MEMORY.md` files, judging what is global (root) vs area specific (nested).
...
Durable context lives in the tool agnostic `MEMORY.md` (root and nested); every skill reads and writes it. `MEMORY.md` and `MEMORY.md` are regenerated by Boost and must not be modified by skills — they contain only the Boost guidelines. All project knowledge, conventions, and commands go into `MEMORY.md`.

- Write knowledge into `MEMORY.md`. Create it when missing. Never overwrite an existing `MEMORY.md` (it may be user or tool authored); gap fill conservatively, with permission.
- Migrate legacy content: a `MEMORY.md` with content but no `MEMORY.md` → ask permission, move its content into a new `MEMORY.md`; let Boost regenerate MEMORY.md. Never silently discard curated content.
- Root stays short and global; nested `MEMORY.md` only for meaningful areas with real conventions (same root/nested rules as previously applied to `MEMORY.md`).

## Scope

An area path argument (e.g. `auth`, `src/payments`) triggers Phase 3. With no argument, the `Pre-flight` signals below route to Phase 0 (ambiguous: ask new vs existing), Phase 1 (greenfield: ask standards, seed root), Phase 2 (established, no root MEMORY.md: whole-repo scan), or Phase 4 (root MEMORY.md exists: gap-fill). A legacy `MEMORY.md` with content but no `MEMORY.md` is migrated, then treated as Phase 4.

## Acts vs asks

Phase 1 asks coding standards questions via MCQ before creating root MEMORY.md. Phase 2 acts immediately, no questions; it writes root and the nested docs it judges warranted. Phases 3 and 4 act to explore but ask permission before modifying an existing root MEMORY.md or migrating a legacy MEMORY.md; Phase 4 reports nested doc creation for undocumented areas first, then applies on confirmation.

## Artifact ownership

The `MEMORY.md` files hold the content: create root if missing (Phase 1, 2) and `<area>/MEMORY.md` if missing and warranted, by judgment (Phase 2, 3, 4); when one exists, gap-fill or propose additions with permission; never overwrite. A legacy `MEMORY.md` with content is migrated into `MEMORY.md` with permission. When creating a nested `MEMORY.md`, add exactly one pointer line to root `MEMORY.md` under `## Context files`: `- [<area>/MEMORY.md](<area>/MEMORY.md) (<one-line description>)`. Never one per subfolder, only where distinct conventions exist.
...
1. Context files: root MEMORY.md present → `ROOT_EXISTS`; a MEMORY.md with content only → `ROOT_LEGACY`; neither → `ROOT_MISSING`.
...
Why this order: the workflow scaffolds a greenfield project (manifest + starter files) BEFORE `/audit` runs, so a raw code count alone would wrongly read a fresh scaffold as an existing codebase and skip the coding standards questions. The workflow setup signal (a stack spec with no root MEMORY.md) catches exactly that case and routes it to greenfield. Only when there is real feature code, real history, and no in-progress workflow setup is it truly brownfield (Phase 2). Anything ambiguous falls to Phase 0 and asks; never default a maybe greenfield project to brownfield.

Monorepo (`MONOREPO=yes`): root plus a light stub per workspace, deepen on demand. Each workspace (`apps/*`, `packages/*`) is a first class area; its primary doc lives at the workspace root (`packages/api/MEMORY.md`), never buried deeper. A whole-repo run does not deep scan every workspace (too expensive, premature); write the repo root `MEMORY.md` (monorepo wide tooling, shared conventions) plus a light stub `MEMORY.md` per workspace from its manifest, no code scan, with root pointers. The full conventions/gotchas/key files scan happens when the engineer runs `/audit packages/api` (Phase 3) or first builds there, never all upfront. A spot inside a workspace that warrants its own doc (`packages/ui/src/mdx/`) gets one in addition to the workspace root doc, linked from it. A doc already there, buried in a workspace with no workspace root doc: ask: "`packages/ui` has a context file at `src/mdx/` but none at its root. Move it up to `packages/ui/MEMORY.md`, or keep it as a nested doc under a new `packages/ui/MEMORY.md`?" On move: relocate it to the workspace root. On keep nested: create the workspace root `MEMORY.md` AND keep the deep one, linking it from the root doc. Migrate legacy `MEMORY.md` content per the convention above. Apply `MONOREPO=yes` plus the workspace list as you write.

Legacy migration (any phase): on `ROOT_LEGACY`, before proceeding ask permission: "I found a `MEMORY.md` with project context but no `MEMORY.md`. I'll move its content into a new `MEMORY.md` (so all tools read it) and replace `MEMORY.md` with a pointer. Proceed?" On yes: copy the content verbatim into `MEMORY.md`, then replace `MEMORY.md` with the pointer; `MEMORY.md` now exists, so continue as Phase 4 (gap-fill). On no: leave both untouched and continue without migrating. Same for any nested `<area>/MEMORY.md`.
...
If no `MEMORY.md` was written when it should have been (the file is missing/empty), report the failure and do it again; don't relay success it didn't produce. Otherwise relay the report: what was discovered (2 to 4 bullets), what was written (file paths), what was proposed or skipped (if existing files were found).
...
See `agent-prompt.md` (the main thread reads it at write time; its per phase instructions and `MEMORY.md` templates).
